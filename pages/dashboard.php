...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <section aria-label="Welcome Banner" class="bg-gradient-to-r from-blue-600 to-teal-500 rounded-2xl p-8 text-white shadow-xl shadow-blue-200 mb-10 relative overflow-hidden">
        <div class="relative z-10 max-w-2xl fade-up">
            <h1 class="text-3xl md:text-4xl font-extrabold tracking-tight">Halo, <?= htmlspecialchars($_SESSION['nama']); ?> !</h1>
            ...
            <div class="mt-6 flex flex-wrap gap-3">
                <?php if($role == 'user'): ?>
                    <button onclick="toggleModalJanji()" class="inline-flex items-center gap-2 px-4 py-2 bg-white text-teal-600 rounded-xl font-semibold shadow hover:scale-105 transition transform">Buat Janji</button>
                <?php else: ?>
                    <a href="edit_detail_balita.php" class="inline-flex items-center gap-2 px-4 py-2 bg-white text-teal-600 rounded-xl font-semibold shadow hover:scale-105 transition transform">Kelola Penimbangan</a>
                <?php endif; ?>
            </div>
        </div>

        <svg class="absolute right-[-10%] top-0 h-full w-1/2 text-white opacity-10 float-up" viewBox="0 0 100 100" fill="currentColor"><circle cx="50" cy="50" r="50"/></svg>
        <div class="absolute -left-6 bottom-0 w-72 h-72 rounded-full bg-white/5 blur-3xl float-up" style="filter: blur(50px); opacity:0.5;"></div>
    </section>
        ...
        <section aria-labelledby="daftar-anak-title" class="mb-12">
            <header class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                ...
                <div class="flex gap-3">
                    <button onclick="toggleModal()" class="inline-flex items-center justify-center px-4 py-2 border border-blue-600 text-sm font-medium rounded-xl text-blue-600 bg-white hover:bg-blue-50 transition shadow-sm">
                    ...
                    <button onclick="toggleModalJanji()" class="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-xl text-white bg-blue-600 hover:bg-blue-700 transition shadow-sm shadow-blue-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                ?>
                <article class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all group">
                    <div class="p-6">
                        <div class="flex items-center space-x-4">
                            <div class="<?= $anak['jenis_kelamin'] == 'L' ? 'bg-blue-50 text-blue-500' : 'bg-pink-50 text-pink-500'; ?> h-14 w-14 rounded-full flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            </div>
                            <div>
                                <h4 class="text-lg font-bold text-gray-900 group-hover:text-blue-600 transition"><?= htmlspecialchars($anak['nama_balita']); ?></h4>
                                <span class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded"><?= $umur->y; ?> Thn <?= $umur->m; ?> Bln</span>
                            </div>
                        </div>
                    </div>
                    <a href="detail_anak.php?id=<?= $anak['id']; ?>" class="block bg-gray-50 hover:bg-blue-50 p-3 text-center text-sm font-semibold text-blue-600 border-t border-gray-100 transition">Lihat Detail →</a>
                </article>
                <?php endwhile; ?>
            </div>
        </section>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Anak & Imunisasi</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            ?>
                            <tr>
                                <td class="px-6 py-4 text-sm text-gray-900"><?= date('d M Y', strtotime($j['tanggal_rencana'])); ?></td>
                                <td class="px-6 py-4 text-sm text-gray-600">
                                    <div class="font-medium text-gray-900"><?= $j['nama_balita']; ?></div>
                                    <div class="text-xs"><?= $j['nama_imunisasi']; ?></div>
                                </td>
                                <td class="px-6 py-4"><span class="px-2 py-1 text-xs font-bold rounded-full <?= $color; ?>"><?= $j['status']; ?></span></td>
                            </tr>
                            <?php endwhile; ?>
                        </tbody>
                    </table>
    ...
    <?php else: ?>
        ...
        <section aria-labelledby="admin-menu-title" class="mb-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <a href="edit_detail_balita.php" class="flex items-center p-6 bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:border-blue-200 transition group">
                    <div class="h-12 w-12 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center mr-4 group-hover:bg-blue-600 group-hover:text-white transition">
                        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3" /></svg>
                    </div>
                    <div>
                ...
                <a href="#" class="flex items-center p-6 bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:border-green-200 transition group">
                    <div class="h-12 w-12 bg-green-50 text-green-600 rounded-xl flex items-center justify-center mr-4 group-hover:bg-green-600 group-hover:text-white transition">
                ...
                <a href="data_anak.php" class="flex items-center p-6 bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:border-orange-200 transition group">
                    <div class="h-12 w-12 bg-orange-50 text-orange-600 rounded-xl flex items-center justify-center mr-4 group-hover:bg-orange-600 group-hover:text-white transition">
        ...
        <section id="tabel-janji" aria-labelledby="konfirmasi-janji-title">
            <div class="flex items-center justify-between mb-6">
                ...
                <span class="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded">Total: <?= $pendingCount; ?> Menunggu</span>
            </div>
                ...
                <div class="overflow-x-auto rounded-[20px] bg-[#1D3428]">
                    <table class="min-w-full text-sm">
                        <thead class="bg-[#1D3428] border-b-2 border-[#D0F246]">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Tanggal</th>
                                <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Orang Tua / Anak</th>
                                <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Imunisasi & Catatan</th>
                                <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Status</th>
    ...
    <tr class="hover:bg-gray-50 transition">
        ...
        <td class="px-6 py-4">
            <div class="text-sm font-bold text-gray-900"><?= htmlspecialchars($row['nama_ortu']); ?></div>
            <div class="text-xs text-gray-500">Anak: <?= htmlspecialchars($row['nama_balita']); ?></div>
        </td>
        <td class="px-6 py-4">
            <span class="text-sm text-gray-700 bg-gray-100 px-2 py-1 rounded"><?= $row['nama_imunisasi']; ?></span>
            <?php if($row['catatan']): ?>
                <p class="text-xs text-gray-400 mt-1 italic">"<?= htmlspecialchars($row['catatan']); ?>"</p>
            <?php endif; ?>
        </td>
            ...
            <?php if($row['status'] != 'Selesai' && $row['status'] != 'Batal'): ?>
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="bukaModalSelesai(
                        ...
                        '<?= $row['nama_imunisasi']; ?>'
                    )" class="bg-green-100 text-green-600 p-2 rounded-lg hover:bg-green-600 hover:text-white transition" title="Selesaikan & Catat Riwayat">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                    </button>

                    <form method="POST" action="" class="flex items-center">
                        <input type="hidden" name="janji_id" value="<?= $row['id']; ?>">
                        <select name="status_baru" onchange="this.form.submit()" class="text-xs border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 py-1.5 ml-1">
...
<div id="modalSelesai" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity backdrop-blur-sm" onclick="tutupModalSelesai()"></div>
            ...
            <form method="POST" action="">
                ...
                <div class="px-6 py-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="tgl_suntik" value="<?= date('Y-m-d'); ?>" required class="block w-full border border-gray-300 rounded-lg shadow-sm py-2 px-3 focus:ring-green-500 focus:border-green-500 sm:text-sm">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Bidan / Petugas</label>
                        <input type="text" name="bidan_penyuntik" placeholder="Contoh: Bidan Ani" required class="block w-full border border-gray-300 rounded-lg shadow-sm py-2 px-3 focus:ring-green-500 focus:border-green-500 sm:text-sm">
                    </div>
                </div>

                <div class="bg-gray-50 px-6 py-4 flex flex-row-reverse gap-3">
                    <button type="submit" name="proses_imunisasi" class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none sm:w-auto sm:text-sm">
                        Simpan & Selesaikan
                    </button>
                    <button type="button" onclick="tutupModalSelesai()" class="mt-3 w-full inline-flex justify-center rounded-xl border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:mt-0 sm:w-auto sm:text-sm">
                        Batal
                    </button>
                </div>
            </form>
